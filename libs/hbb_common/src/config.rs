...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("Quinyx RustDesk".to_owned());
    static ref KEY_PAIR: Mutex<Option<KeyPair>> = Default::default();
    static ref USER_DEFAULT_CONFIG: RwLock<(UserDefaultConfig, Instant)> = RwLock::new((UserDefaultConfig::load(), Instant::now()));
    pub static ref NEW_STORED_PEER_CONFIG: Mutex<HashSet<String>> = Default::default();
    pub static ref DEFAULT_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    // Locked settings baked at compile time from CI secrets. Options present here
    // take priority on read and `is_option_can_save` refuses to overwrite them,
    // so the end user cannot change the server, key or auth method.
    pub static ref OVERWRITE_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new(baked_overwrite_settings());
    pub static ref DEFAULT_DISPLAY_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    pub static ref OVERWRITE_DISPLAY_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    pub static ref DEFAULT_LOCAL_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    pub static ref OVERWRITE_LOCAL_SETTINGS: RwLock<HashMap<String, String>> = Default::default();
    // Preset permanent password (plaintext, no salt) baked from the PRESET_PASSWORD secret.
    pub static ref HARD_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new(baked_hard_settings());
    // Disable changing the permanent password when a preset password is baked in.
    pub static ref BUILTIN_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new(baked_builtin_settings());
}

// Value of a compile-time env var (CI secret), trimmed; `None` if unset or empty.
fn baked_env(value: Option<&'static str>) -> Option<String> {
    match value {
...
// Server, public key and auth method baked from CI secrets and locked (read-only for the user).
fn baked_overwrite_settings() -> HashMap<String, String> {
    let mut m = HashMap::new();
    if let Some(v) = baked_env(option_env!("RENDEZVOUS_SERVER")) {
        m.insert("custom-rendezvous-server".to_owned(), v);
    }
    if let Some(v) = baked_env(option_env!("RS_PUB_KEY")) {
        m.insert("key".to_owned(), v);
    }
    if let Some(v) = baked_env(option_env!("API_SERVER")) {
        m.insert("api-server".to_owned(), v);
    }
    // When a preset password is baked in, force permanent-password-only auth so the
    // predefined password is always the way in (no rotating temporary password).
    if baked_env(option_env!("PRESET_PASSWORD")).is_some() {
        m.insert(
...
// Preset permanent password (plaintext, no salt) baked from the PRESET_PASSWORD secret.
fn baked_hard_settings() -> HashMap<String, String> {
    let mut m = HashMap::new();
    if let Some(v) = baked_env(option_env!("PRESET_PASSWORD")) {
        m.insert("password".to_owned(), v);
    }
    m
}

// Lock the permanent password so it cannot be changed when one is baked in.
fn baked_builtin_settings() -> HashMap<String, String> {
    let mut m = HashMap::new();
    if baked_env(option_env!("PRESET_PASSWORD")).is_some() {
        m.insert(
...
// Baked at compile time from the `RENDEZVOUS_SERVER` / `RS_PUB_KEY` env vars
// (set via GitHub Actions secrets); fall back to the public defaults otherwise.
